...
namespace TGS {
    ...
    public partial class TerrainGridSystem : MonoBehaviour {
        ...
        public float cellBorderThickness {
            ...
            set {
                if (value != _cellBorderThickness) {
                    _cellBorderThickness = Mathf.Max(1f, value);
                    if (_showCells)
                        DrawCellBorders();
                    isDirty = true;
                }
        ...
        /// <summary>
        /// Returns the cell index of a cell at a given local position in the grid
        /// </summary>
        public int CellGetIndex(Vector2 localPosition) {
            Cell cell = CellGetAtPosition(localPosition);
        ...
        /// Get a list of cells which are nearer than a given distance in cell count
        /// </summary>
        public List<int> CellGetNeighboursWithinRange(int cellIndex, int minSteps, int maxSteps, int cellGroupMask = -1, int maxCost = -1, bool includeInvisibleCells = true, CanCrossCheckType canCrossCheckType = CanCrossCheckType.Default, bool ignoreCellCosts = false) {
            if (cellIndex < 0 || cellIndex >= cells.Count)
                return null;
        ...
        public GameObject CellSetMaterial(int cellIndex, Material material) {
            ...
            GameObject o = CellToggleRegionSurface(cellIndex, true, Color.white);
            Region region = cells[cellIndex].region;
        ...
        public int GetCellsWithinCone(int cellIndex, int targetCellIndex, float angle, List<int> cellIndices) {
            ...
            if (cellIndex < 0 || cellIndex >= cells.Count) return -1;
            if (targetCellIndex < 0 || targetCellIndex >= cells.Count) return -1;
        ...
        public bool CellSetTerritory(int cellIndex, int territoryIndex) {
            ...
            if (cellIndex < 0 || cellIndex >= cells.Count || territoryIndex >= terrCount)
                return false;
            Cell cell = cells[cellIndex];
