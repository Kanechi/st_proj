...
namespace TGS {
    ...
    public partial class TerrainGridSystem : MonoBehaviour {
        ...
        /// </summary>
        void SurfaceSegmentForFlatMesh(Vector3 p0, Vector3 p1, float height, Vector3 invNormal) {
                ...
                for (int k = 0; k < cellCount; k++) {
                    ...
                    if (cell != null && cell.visible && cell.borderVisible) {
                        ...
                        if (_cellsFlat) {
                            ...
                            for (int i = 0; i < numSegments; i++) {
                                Segment s = region.segments[i];
                                SurfaceSegmentForFlatMesh(GetScaledVector(s.start.vector3), GetScaledVector(s.end.vector3), height, invNormal);
                            }
                        } else {
                            for (int i = 0; i < numSegments; i++) {
                                Segment s = region.segments[i];
        ...
        void BuildTerrainWrapper() {
            ...
            if (_terrainObject == null) {
                ...
                _terrainWrapper = TerrainWrapperProvider.GetTerrainWrapper(_terrainObject, _terrainObjectsPrefix, _terrainObjectsLayerMask, _terrainObjectsSearchGlobal, _heightmapSize, _heightmapSize);
            }
        }
        ...
        void DrawCellBorders() {
            ...
            float thick = (_cellBorderThickness - 0.99f) * transform.lossyScale.x / 500f;
        ...
        void DrawTerritoryFrontiers() {
            ...
            territoriesGeoMat.SetFloat(ShaderParams.Thickness, thick);
            territoriesDisputedGeoMat.SetFloat(ShaderParams.Thickness, thick);
        }
        ...
        Poly2Tri.Polygon GetPolygon(Region region, ref PolygonPoint[] polyPoints, out int pointCount) {
            ...
            if (_gridScale.x == 1f && _gridScale.y == 1f && _gridCenter.x == 0 && _gridCenter.y == 0) {
                ...
                if (_cellsFlat || _terrainWrapper == null) {
                    for (int i = 0; i < numSegments; i++) {
                        Segment s = region.segments[i];
                        tempConnector.Add(s);
                    }
        ...
        bool GetLocalHitFromMousePos(out Vector3 localPoint) {
            ...
            if (useEditorRay && !Application.isPlaying) {
                ...
                //if (!mouseIsOver && !Application.isMobilePlatform) { // TODO: remove -- this causes a regression by which highlight point is offset in 3d terrain/meshes
                //    if (_blockingMask != 0 && IsRayBlocked(ray, float.MaxValue)) return false;
